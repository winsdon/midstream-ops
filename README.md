...
| 问题           | 本项目的回答                                                                               |
| ------------ | ------------------------------------------------------------------------------------ |
| **我到底赚不赚钱？** | 收益取本站实扣，成本取**上游 per-key 真实实扣**（不是推算），逐供应商 / 逐分组算利润。上游按各自计价单位记账，**统一按站点充值倍率折成人民币**再相减 |
...
- **仪表盘** — 今日收益 / 实扣成本 / 利润 / 请求数总览，近 N 天趋势图与分组贡献榜，成本同步状态与完整性告警
- **上游管理** — 显式关联账号；多平台（sub2api / new-api）多认证模式；余额采集与历史曲线；上游 per-key 成本明细（实扣 / 上游成本 / 折扣节省）；自营站标记与运营成本记账
...
- **模型检测** — 对上游 Claude 渠道做指纹检测：官方 Max 订阅号池 / 官方 API Key / AWS Bedrock / Kiro / Vertex / 包装伪装六选一，并给出独立的「后端是不是真 Claude」真实性评分，历史留痕可对照
...
| 模块         | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **仪表盘**    | 今日收益 / 实扣成本 / 利润 / 请求数 / 供应商数 / 账号数，近 N 天收益·成本·利润趋势图；分组贡献榜（Top6 横向条形 + Top3 集中度占比）；成本同步状态与完整性告警条（含 key 匹配率）                                                                                                                                                                                                                                                                                                                                        |
| **上游管理**   | **显式关联账号**（按站点地址归组多选，支持按账号名 / 平台 / 站点地址 / 归属站点 / ID 搜索与「全选本组」；或按账号名【】前缀批量建站顺带关联）；CRUD；多平台（sub2api / new-api）多认证模式（账号密码 / Access+Refresh Token / 系统令牌+用户 ID）；按平台 / 状态 / 余额类型三维筛选，按今日消费·余额·名称排序（默认今日消费降序，**「不监控」站点恒排最后**）；采集健康点（绿/黄/红）与登录冷却提示；站点级忽略余额告警开关；手动录入余额；余额历史曲线；可用分组按平台（Anthropic / OpenAI / Gemini …）分节展示；💰 上游成本明细（per-key 实扣 / 上游成本 / 折扣节省，手动同步与回补）；**自营站标记**与运营成本记账（买号 / 订阅 / 服务器）                                                       |
...
| **稳定性**    | 被动流量色块矩阵：顶栏 KPI、按供应商/分组的可用性趋势、悬停看桶内明细、点击看账号弹窗、顶部排序（成功率 / 请求数 / 首字 / 吞吐 / 缓存）。窗口 5 分钟 / 30 分钟 / 1 小时 / 6 小时 / 24 小时，默认 1 小时。探测接口仍保留，本页不再入口。**详见** **[稳定性可视化](docs/DESIGN_NOTES.md#稳定性可视化)**                                                                                                                                                  |
| **模型检测**   | 对上游 Claude 渠道做指纹检测，可从本站 anthropic 账号勾选（密钥不出后端）或手填地址与 Key，一次最多 10 个目标。检测项分四组：连通与网关指纹（消息 id 形态、注入量、限流头）、协议严格性（非法参数是否被拒、max\_tokens 硬上限、tool\_use id 前缀、thinking 签名原样回传与篡改拒绝、SSE 状态机、count\_tokens 一致性）、身份与人设（Claude Code / Kiro 归属、工作区泄露、系统提示泄露、型号与知识截止、system 穿透）、能力与采样（图片 / PDF / 严格 Schema / Prompt Cache / 采样熵 / thinking 梯度 / 输出斜率，默认关闭）。跑完后另有一道**零请求的跨请求审计**：缓存门槛、usage 守恒、thinking 自洽、平台形态交叉一致 —— 这些矛盾只在跨请求比对时才暴露。输出**渠道分类 + 真实性评分**双结论，逐项可下钻到脱敏请求与原始响应，结果落库可对照历史。**详见** **[模型检测（渠道指纹）](docs/DESIGN_NOTES.md#模型检测渠道指纹)** |
...
> **📐 每个数字背后的口径、取舍与实现约束，见** **[docs/DESIGN\_NOTES.md](docs/DESIGN_NOTES.md)** —— 成本为什么取上游实扣而非本站推算、分组利润合计为什么偏高、稳定性色点为什么取最差而非平均，都在那里。
...
│       ├── handler/        HTTP 处理器
│       ├── service/        业务逻辑（余额采集 / 探测 / 倍率 / 统计 / 调度 / 授信台账 / 渠道检测）
...
| GET             | `/api/v1/providers/scan-urls`                             | 按账号 base\_url 归组（不依赖命名习惯）                      |
...
| GET             | `/api/v1/providers/:id/costs?start=&end=`                 | 上游 per-key 成本明细（实扣 / 上游成本 / 匹配状态）              |
...
| 文档                                                   | 内容                                         |
| ---------------------------------------------------- | ------------------------------------------ |
| [docs/DESIGN\_NOTES.md](docs/DESIGN_NOTES.md)        | 口径与设计说明 —— 收益成本利润口径、稳定性可视化取舍、授信台账约束、渠道检测判据 |
| [deploy/DOCKER.md](deploy/DOCKER.md)                 | Docker 部署详解（网络、两套库账号、密钥、备份）                |
| [deploy/UPGRADE\_TO\_PG.md](deploy/UPGRADE_TO_PG.md) | 从 0.1.x SQLite 版升级到 PostgreSQL             |
