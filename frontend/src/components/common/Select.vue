<template>
  <div class="relative" ref="containerRef">
    ...
    <!-- 浮层 Teleport 到 body，避开 BaseDialog / card 的 overflow 与层叠上下文 -->
    <Teleport to="body">
...
// 每个实例一个唯一 class，用于点击外部检测——浮层被 Teleport 走了，
// 无法靠 containerRef.contains 判断。
...
const searchPlaceholderText = computed(
  () => props.searchPlaceholder ?? t('common.searchOptionPlaceholder')
)
...
const selectedOption = computed(
  () => props.options.find((opt) => getOptionValue(opt) === props.modelValue) || null
)
...
<style>
/* 非 scoped：浮层 Teleport 到 body 后不在组件作用域内 */
