<template>
  <div class="flex flex-col items-center justify-center rounded-xl border border-gray-100 bg-gray-50/60 p-3 dark:border-dark-800 dark:bg-dark-800/40">
    ...
    <!-- 有有效充值倍率时 CNY 主显、上游原值副显；否则原值升为主显 -->
...
const props = withDefaults(
  defineProps<{
    label: string
    /** 上游原始值，单位是该站自己的计价单位（U 或人民币）；null/undefined 表示无数据 */
    usd?: number | null
    /** 充值倍率：上游原值 × rate = CNY；<= 0 视为无效 */
    rate: number
...
const cny = computed(() => {
  ...
  return '¥' + (props.usd! * props.rate).toFixed(2)
})

// 上游原值：倍率非 1 时是供应商自己的计价单位（U），标 USD 会误导，
// 故只在等于 1 时才带单位后缀
const usdText = computed(() => {
  if (!hasValue.value) return '-'
  const raw = trimZeros(props.usd!)
  return props.rate === 1 ? raw + ' USD' : raw + ' U'
})
