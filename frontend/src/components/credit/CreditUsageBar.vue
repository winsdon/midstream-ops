...
const { t } = useI18n()
const { privacyMode } = usePrivacyMoney()
...
// 超额时条子填满即可，不撑破容器。隐私模式下条宽也是模拟量泄露，必须打码。
const barWidth = computed(() => {
  if (privacyMode.value) return '0%'
  return `${Math.min(Math.max(percent.value, 0), 100)}%`
})

// 必须写完整字面量：Tailwind 扫描源码文本提取类名，拼接出来的类名会被漏掉
const barClass = computed(() => {
  if (privacyMode.value || !granted.value) return 'bg-gray-300 dark:bg-dark-600'
...
const labelClass = computed(() => {
  if (privacyMode.value || !granted.value) return 'text-gray-400'
...
const label = computed(() => {
  if (!granted.value) return t('credit.notGranted')
  if (privacyMode.value) return '****'
  return fmtPct(percent.value)
})
</script>
